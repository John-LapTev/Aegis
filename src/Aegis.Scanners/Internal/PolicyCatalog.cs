...
public static class PolicyCatalog
{
    ...
    [
        ...
        // ВАЖНО: EnableLUA сюда НЕ добавлять. Это не GPO-надстройка, а само значение UAC: его нельзя «снять»
        // удалением (удаление НЕ включает UAC), а зелёное «включено» после этого было бы враньём. Выключенный
        // UAC ловит и правильно чинит SettingsScanner (settings-uac-off → EnableLUA=1). Найдено аудитом 2026-07-23.
