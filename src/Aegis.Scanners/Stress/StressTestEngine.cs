...
public sealed class StressTestEngine : IStressTestEngine
{
    ...
    private (Severity Severity, string Verdict) BuildVerdict(StressAbortReason reason, int? maxCpu, int? maxGpu)
    {
        ...
        // Completed — прошёл полностью. Тест нагружает ПРОЦЕССОР, поэтому и оценка по его температуре
        // (видеокарту он не грузит — показывать её температуру как «под полной нагрузкой» было бы неправдой).
        var hotCpu = maxCpu is int mc && mc >= _options.CpuWarnCelsius;

        if (maxCpu is null)
        {
            return (Severity.Info,
                "Проверка под нагрузкой прошла полностью, процессор выдержал. Температуру измерить не удалось " +
        ...
        if (hotCpu)
        {
            return (Severity.Warning,
                $"Проверка прошла, процессор выдержал, но под полной нагрузкой грелся заметно{peak}. Пока терпимо, " +
        ...
        return (Severity.Ok,
            $"Отлично: под полной нагрузкой процессор держался спокойно{peak}. Охлаждение справляется, " +
            "беспокоиться не о чем.");
    }

    /// <summary>Хвост вида « (процессор до 78 °C)» — или пусто, если датчика не было. Тест грузит только процессор.</summary>
    private static string PeakText(int? maxCpu, int? maxGpu)
    {
        _ = maxGpu; // видеокарту тест не нагружает — её температуру в вердикт не выносим
        return maxCpu is int c ? $" (процессор до {c} °C)" : string.Empty;
    }
}
